...
/// Defines which APIs are available.
///
/// Has to be bumped whenever a new versioned API is added.
...
impl From<ApiVersion> for crate::wasm::ApiVersion {
	fn from(version: ApiVersion) -> Self {
		Self::Versioned(version.0)
	}
}
...
fn api_version_up_to_date() {
	assert!(
		ApiVersion::default().0 == crate::wasm::HIGHEST_API_VERSION,
		"A new versioned API has been added. The `ApiVersion` needs to be bumped."
	);
}
